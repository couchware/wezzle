...
public class OggInputStream extends InputStream implements AudioInputStream {
	...
	private boolean getPageAndPacket() {
		...
		try {
			...
			CouchLogger.get().recordException(this.getClass(), new Exception("Failure reading in vorbis"));
		...
		if (syncState.pageout(page) != 1) {
			...
			CouchLogger.get().recordException(this.getClass(),
                                new Exception("Input does not appear to be an Ogg bitstream."));
			endOfStream = true;
			return false;
		}
		...
		if (streamState.pagein(page) < 0) {
			...
			CouchLogger.get().recordException(this.getClass(), 
                                new Exception("Error reading first page of Ogg bitstream data."));
			endOfStream = true;
			return false;
		}
		...
		if (streamState.packetout(packet) != 1) {
			...
			CouchLogger.get().recordException(this.getClass(), new Exception("Error reading initial header packet."));
			endOfStream = true;
			return false;
		}
		...
		if (oggInfo.synthesis_headerin(comment, packet) < 0) {
			...
			CouchLogger.get().recordException(this.getClass(), 
                                new Exception("This Ogg bitstream does not contain Vorbis audio data."));
			endOfStream = true;
			return false;
		}
		...
		while (i < 2) {
				...
				if (result == 1) {
					...
					while (i < 2) {
						...
						if (result == -1) {
							...
							CouchLogger.get().recordException(this.getClass(), 
                                                                new Exception("Corrupt secondary header.  Exiting."));
							endOfStream = true;
							return false;
						}
			...
			try {
				...
				CouchLogger.get().recordException(this.getClass(), new Exception("Failed to read Vorbis: "));
				CouchLogger.get().recordException(this.getClass(), e);
				endOfStream = true;
				return false;
			}
			if (bytes == 0 && i < 2) {
				CouchLogger.get().recordException(this.getClass(), 
                                        new Exception("End of file before finding all Vorbis headers!"));
				endOfStream = true;
				return false;
			}
			syncState.wrote(bytes);
			...
			while (!endOfBitStream) {
				while (!endOfBitStream) {
					...
					if (result == -1) { // missing or corrupt data at this page position
						CouchLogger.get().recordException(this.getClass(), 
                                                        new Exception("Corrupt or missing data in bitstream; continuing..."));
					} else {
							...
							if (result == -1) { // missing or corrupt data at this page position
								...
								while ((samples = dspState.synthesis_pcmout(_pcm,
									...
									if (bytesToWrite >= pcmBuffer.remaining()) {
										CouchLogger.get().recordWarning(this.getClass(), "Read block from OGG that was too big to be buffered: " + bytesToWrite);
									} else {
				...
				if (!endOfBitStream) {
					...
					if (index >= 0) {
						...
						try {
							...
							CouchLogger.get().recordException(this.getClass(), 
                                                                new Exception("Failure during vorbis decoding"));
							CouchLogger.get().recordException(this.getClass(), e);
							endOfStream = true;
							return;
						}
