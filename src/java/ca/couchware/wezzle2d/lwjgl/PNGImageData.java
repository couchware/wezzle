 ...
 * 
 * @author Cameron McKay (Wezzle)
 * @author Kevin Glass (Slick)
 * @author Matthias Mann (Original)
 */
public class PNGImageData
{
    ...
    public ByteBuffer loadImage(
            InputStream fis, boolean flipped, 
            int[] transparent) 
            throws IOException
    {
            ...
            throws IOException
    {
        ...
        scratch = BufferUtils.createByteBuffer(textureWidth * textureHeight * perPixel);
        decode(scratch, textureWidth * perPixel, flipped);

        if (!hasAlpha() && forceAlpha)
        {
            ByteBuffer temp = BufferUtils.createByteBuffer(textureWidth * textureHeight * 4);
