...
public class ImageIOImageData
{
    ...
    public ByteBuffer imageToByteBuffer(BufferedImage image, boolean flipped, boolean forceAlpha, int[] transparent)
    {
        ...
        while ( txWidth < image.getWidth() )
        {
            txWidth *= 2;
        }
        while ( txHeight < image.getHeight() )
        {
            txHeight *= 2;
        }
        ...
        this.texHeight = txHeight;
        this.texWidth = txWidth;
        ...
        {
            depth = 32;
            raster = Raster.createInterleavedRaster( DataBuffer.TYPE_BYTE, txWidth, txHeight, 4, null );
            texImage = new BufferedImage( glAlphaColorModel, raster, false, new Hashtable() );
        }
        else
        {
            depth = 24;
            raster = Raster.createInterleavedRaster( DataBuffer.TYPE_BYTE, txWidth, txHeight, 3, null );
        ...
        if ( useAlpha )
        {
            ...
            g.fillRect( 0, 0, txWidth, txHeight );
        }
        ...
        if ( edging )
        {
            ...
            {
                copyArea( texImage, 0, 0, width, 1, 0, txHeight - 1 );
                copyArea( texImage, 0, height - 1, width, 1, 0, 1 );
            }
            if ( width < txWidth - 1 )
            {
                copyArea( texImage, 0, 0, 1, height, txWidth - 1, 0 );
